    ...
    if (!post) {
        return (
            <main className="container mx-auto px-4 py-10">
                ...
                <p className="text-muted-foreground mt-2">The blog post you are looking for does not exist.</p>
            </main>
        );
    }
                ...
                <section className="mt-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {post.images.map((src: string, idx: number) => (
                            <img key={idx} src={src} alt={`Image ${idx + 1}`} className="w-full h-auto rounded-md"/>
                        ))}
                    </div>
                </section>
            )}
                    ...
                    <div className="space-y-4">
                        {post.videos.map((url: string, idx: number) => (
                            <div key={idx} className="aspect-video w-full">
                                <iframe src={url} allow={"true"} className="w-full h-full rounded-md"></iframe>
                            </div>
                        ))}
                    </div>
                </section>
